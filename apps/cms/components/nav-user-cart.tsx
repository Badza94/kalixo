...
import { CircleFlag } from "@workspace/ui/react-circle-flags";
...
function NavUserCart() {
  ...
  const { calculateSubtotal, getDisplayPrice } = useCartProductInputs(cart);
    ...
    <Drawer open={openDialog} onOpenChange={setOpenDialog} direction="right">
      <DrawerTrigger asChild>
        <Button size="icon" variant="ghost" className="relative">
          ...
          <ShoppingBag className="h-8 w-8 !size-5" />
        </Button>
      </DrawerTrigger>
      <DrawerContent className="h-full ml-auto data-[vaul-drawer-direction=right]:w-full data-[vaul-drawer-direction=right]:sm:max-w-md">
        ...
        {cartCount > 0 ? (
          <ScrollArea className="h-[calc(100dvh-220px)] space-y-4 px-4 mb-4">
            {cart.map((product) => {
        ...
        {cartCount > 0 && (
          <DrawerFooter className="py-4 border-t">
            <div className="flex items-center justify-between font-bold">
              <p className="text-lg">{ct("total")}</p>
              <p className="text-xl">
                {formatCurrency(getTotalPrice(), "EUR", locale)}
              </p>
            </div>
            <Button>{ct("checkout")}</Button>
            <Button variant="text" onClick={handleSaveAsDraft}>
              {t("saveAsDraft")}
